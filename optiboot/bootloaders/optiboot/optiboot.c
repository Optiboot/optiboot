 ...
 *   Customizable timeout with accurate time constant
 ...
 *
 * Does not support:
 *   USB based devices (e.g., Teensy, Leonardo)
 *
 * Assumptions:
...
/* C zero initializes all global variables.
 * These definitions are NOT zero initialized, but that doesn't matter.
 * This allows us to drop the zero init code, saving us memory */
#define GLOBAL_STORAGE_SIZE_BYTES 2
#define global_space    ((uint8_t*)(RAMSTART))
#define buff    ((uint8_t*)(RAMSTART + GLOBAL_STORAGE_SIZE_BYTES))

/* Virtual boot partition support */
#ifdef VIRTUAL_BOOT_PARTITION
   #define rstVect0_sav (*(uint8_t*)(RAMSTART + GLOBAL_STORAGE_SIZE_BYTES + SPM_PAGESIZE * 2 + 4))
   #define rstVect1_sav (*(uint8_t*)(RAMSTART + GLOBAL_STORAGE_SIZE_BYTES + SPM_PAGESIZE * 2 + 5))
   #define saveVect0_sav (*(uint8_t*)(RAMSTART + GLOBAL_STORAGE_SIZE_BYTES + SPM_PAGESIZE * 2 + 6))
   #define saveVect1_sav (*(uint8_t*)(RAMSTART + GLOBAL_STORAGE_SIZE_BYTES + SPM_PAGESIZE * 2 + 7))
...
#define tries (0)    /* first byte of global_space */
...
int main(void)
{
	...
	if(reset_type & (_BV(WDRF) | _BV(BORF))) /* Watchdog and Brown-out resets are ignored */
	{
		appStart(reset_type);
	}

	/* Reset pin and power-on resets can trigger bootloading */
	...
	if(reset_type == 0) /* debugger reset */
	{
 ...
 */
					if((uint16_t)(void*)address == rstVect0)
					{
						...
						vect = (vect - save_vect_num) & 0x0fff;                                 /*subtract 'save' interrupt position and wrap around 4096 */
						/* Move RESET jmp target to 'save' vector */
						buff[saveVect0] = vect & 0xff;
						buff[saveVect1] = (vect >> 8) | 0xc0;   /* */
						/* Add rjump to bootloader at RESET vector */
						vect = ((uint16_t)main) & 0x0fff;       /*WARNING: this works as long as 'main' is in first section */
		...
		else if(ch == STK_READ_PAGE)            /* Read memory block mode, length is big endian.  */
		{
			uint8_t desttype;
			GETLENGTH(length);
...
uint8_t getch(void)
{
	uint8_t ch;
		...
		if(!(UART_SRA & _BV(FE0)))
		{
			...
			/* POR may result in situations with frame errors - so keep trying for a while */
			if(!global_space[tries])
			{
				watchdogReset();
			}
...
void __attribute__((optimize("O1"))) watchdogReset()
{
	__asm__ __volatile__ (
		"wdr\n"
		);
...
void __attribute__((optimize("O1"))) watchdogConfig(uint8_t x)
{
...
static inline void read_mem(uint8_t memtype, uint16_t address, pagelen_t length)
{
	...
	default:
		do
		{
				...
				else if(address == rstVect1)
				{
					ch = rstVect1_sav;
				}
				else if(address == saveVect0)
				{
					ch = saveVect0_sav;
				}
				else if(address == saveVect1)
				{
					ch = saveVect1_sav;
				}
				else
